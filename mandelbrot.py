...
# ---------------------------------------------------------------------
limites_originales = (np.float64(-2.25), np.float64(1.25), np.float64(-1.5), np.float64(1.5))
...
animando = False
objetivo_pendiente = None   # Almacena el siguiente objetivo si se pulsa durante una animación
...
imagen_mandelbrot = generar_mandelbrot(x_min, x_max, y_min, y_max, ancho, alto, iter_max)
imagen_objeto = eje.imshow(imagen_mandelbrot, extent=(x_min, x_max, y_min, y_max),
...
def actualizar_fractal(baja_resolucion=True):
    ...
    imagen = generar_mandelbrot(x_min, x_max, y_min, y_max, resolucion[0], resolucion[1], iter_max)
...
def animar_secuencia_zoom(limites_objetivo, pasos=60, retardo=0.01):
    ...
    while objetivo_actual is not None:
        ...
        for paso in range(pasos):
            ...
            x_min, x_max, y_min, y_max = nuevo_x_min, nuevo_x_max, nuevo_y_min, nuevo_y_max
...
def animar_zoom_dinamico(limites_objetivo, pasos_totales=120, retardo=0.005):
    ...
    centro_objetivo = ((limites_objetivo[0] + limites_objetivo[1]) / 2,
...
def zoomear(factor, pasos=20, retardo=0.01):
    ...
    limites_objetivo = (centro_x - ancho_objetivo / 2, centro_x + ancho_objetivo / 2,
                        centro_y - alto_objetivo / 2, centro_y + alto_objetivo / 2)
...
control_iter = Slider(eje_iter, "Iter Máx", 100, 3000, valinit=iter_max, valstep=10)
...
especificaciones_botones = [
    ("Mandelbrot Original", (np.float64(-2.25), np.float64(1.25),
                             np.float64(-1.5), np.float64(1.5))),
    ("Minibrot", (np.float64(-1.943), np.float64(-1.94),
                  np.float64(-0.0012), np.float64(0.0012))),
    ("Bulb", (np.float64(-1.764), np.float64(-1.7527),
              np.float64(-0.01925), np.float64(-0.0109))),
...
for spec in especificaciones_botones:
    ...
    eje_boton = plt.axes([0.82, 0.75 - especificaciones_botones.index(spec) * 0.07, 0.15, 0.05])
    boton = Button(eje_boton, etiqueta)
    # Se usa iniciar_animacion_dinamica en lugar de la función anterior
    boton.on_clicked(lambda evento, l=limites: iniciar_animacion_dinamica(l, pasos=60, retardo=0.01))
