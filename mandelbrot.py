...
def experiment1(event):
    if is_animating: 
        return
    animate_zoom_to(-2.25, 1.25, -1.5, 1.5, steps=20)  # Zoom amplio con menos detalle

def experiment2(event):
    if is_animating: 
        return
    animate_zoom_to(-1.943, -1.94, -0.0012, 0.0012, steps=30)  # Zoom medio

def experiment3(event):
    if is_animating: 
        return
    animate_zoom_to(-1.764, -1.7527, -0.01925, -0.0109, steps=40)  # Zoom más cercano

def experiment4(event):
    if is_animating: 
        return
    animate_zoom_to(-1.768562608, -1.7685626045, -0.000790008, -0.000790005, steps=100)  # Zoom ultra-detalle
